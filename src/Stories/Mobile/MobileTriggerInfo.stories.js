...
var MobileTriggerInfo_1 = __importDefault(require("../../Components/Mobile/MobileTriggerInfo/MobileTriggerInfo"));
var sourceData = {
...
var stories = [{
        ...
        triggerState: __assign(__assign({}, triggerState), { state: "EXCEPTION", msg: "Some error very very very very very very very very very very very message message message message message." }),
        data: sourceData
    }, {
        title: "WithLongDescription",
        triggerState: __assign({}, triggerState),
        data: __assign(__assign({}, sourceData), { desc: "Some error " + Array(100).fill("very").join(" ") + " long description" })
    }, {
        title: "WithLongName With spaces",
        triggerState: __assign({}, triggerState),
        data: __assign(__assign({}, sourceData), { name: "Some very very very very very very very very very long name" })
    }];
var story = react_1.storiesOf("Mobile/TriggerInfo", module).addDecorator(storybook_react_router_1.default());
stories.forEach(function (_a) {
    var title = _a.title, data = _a.data, state = _a.triggerState;
    story.add(title, function () { return React.createElement(MobileTriggerInfo_1.default, { triggerState: state, data: data, onThrottlingRemove: addon_actions_1.action("onThrottlingRemove"), onSetMaintenance: addon_actions_1.action("onSetMaintenance") }); });
});
