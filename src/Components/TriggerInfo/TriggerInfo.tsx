  ...
  return <section>
            <header className={cn("header")}>
                ...
                <div className={cn("controls")}>
                    {throttling !== 0 && <span className={cn("control")}>
                            <Button use="link" icon={<ClearIcon />} onClick={() => onThrottlingRemove(id)}>
                                Disable throttling
                            </Button>
                        </span>}
                    ...
                    <span className={cn("control")}>
                        <Dropdown use="link" caption={maintenanceCaption(delta)}>
                            {Object.keys(Maintenances).map(key => <MenuItem key={key} onClick={() => onSetMaintenance(key)}>
                    ...
                    <span>
                        {delta > 0 && maintenanceInfo && maintenanceInfo.setup_user && maintenanceInfo.setup_time && <Tooltip render={() => <div>
                                            ...
                                            <br />
                                            at{" "}
                                            {format(fromUnixTime(maintenanceInfo.setup_time), "MMMM d, HH:mm:ss")}
                                        </div>}>
            ...
            <dl className={cn("list")}>
                ...
                <dd>
                    {targets.map(target => <div key={target}>{target}</div>)}
                </dd>
                {desc && <dt>Description</dt>}
                {desc && <dd className={cn("description", "wysiwyg")} dangerouslySetInnerHTML={{
        __html: sanitize(md.render(desc), purifyConfig)
                ...
                {(state === "EXCEPTION" || state === "ERROR") && <dd className={cn("exception-explanation")}>
