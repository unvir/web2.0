...
class TriggerListPage extends React.Component<Props, State> {
  ...
  async loadData() {
    ...
    const locationSearch = TriggerListPage.parseLocationSearch(location.search);
    const redirected = this.loadLocalSettingsAndRedirectIfNeed(locationSearch.tags, locationSearch.onlyProblems);

    if (redirected) return;

    try {
      const [settings, triggers, tags] = await Promise.all([moiraApi.getSettings(), moiraApi.getTriggerList(transformPageFromHumanToProgrammer(locationSearch.page), locationSearch.onlyProblems, locationSearch.tags, locationSearch.searchText), moiraApi.getTagList()]);
  ...
  loadLocalSettingsAndRedirectIfNeed(tags: Array<string>, onlyProblems: boolean) {
    ...
    const {
      tags: localTags,
      onlyProblems: localOnlyProblems
  ...
  changeLocationSearch = update => {
    ...
    history.push(`?${queryString.stringify(settings, {
      arrayFormat: "index",
      encode: true
    })}`);
  };
