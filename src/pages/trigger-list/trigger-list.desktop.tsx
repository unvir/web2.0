    ...
    return <Layout loading={loading} error={error}>
                <LayoutPlate>
                    <RowStack verticalAlign="baseline" block gap={3}>
                        <Fill>
                            <SearchSelector search={searchText} selectedTokens={selectedTags} subscribedTokens={difference(subscribedTags, selectedTags)} remainingTokens={difference(allTags, selectedTags)} onChange={this.handleChange} onSearch={this.handleSearch} />
                        </Fill>
                        <Fit>
                            <Toggle checked={onlyProblems} onValueChange={value => onChange({ onlyProblems: value })} />{" "}
                            Only Problems
                        </Fit>
                    </RowStack>
                </LayoutPlate>
                <LayoutContent>
                    <ColumnStack block gap={6} horizontalAlign="stretch">
                        ...
                        <TriggerList searchMode={searchText !== ""} items={triggers} onChange={onSetMetricMaintenance} onRemove={onRemoveMetric} />
                    </ColumnStack>
                </LayoutContent>
                {pageCount > 1 && <LayoutFooter>
                        <Paging caption="Next page" activePage={activePage} pagesCount={pageCount} onPageChange={page => onChange({ page })} withoutNavigationHint />
                    </LayoutFooter>}
            </Layout>;
  }
