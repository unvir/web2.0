...
export default function TagList(props: Props): React.ReactNode {
  ...
  return <div>
            ...
            {items.map(item => <TagListItem key={item.name} data={item} allContacts={contacts} onRemove={() => onRemove(item.name)} onRemoveContact={id => onRemoveContact(id)} />)}
        </div>;
}
...
class TagListItem extends React.Component<ItemProps, ItemState> {
  ...
  render(): React.ReactNode {
    ...
    return <div className={cn("row", { active: showInfo, clicable: isSubscriptions })}>
                {isSubscriptions ? <button type="button" className={cn("name", "clicked")} onClick={() => this.setState({ showInfo: !showInfo })}>
                        {name}
                    </button> : <div className={cn("name")}>{name}</div>}
                ...
                {showInfo && <div className={cn("info")}>
                        {isSubscriptions && <div className={cn("group")}>
                                {subscriptions.map(({
            id,
            enabled,
            user,
