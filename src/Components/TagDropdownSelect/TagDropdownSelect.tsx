...
export default class TagDropdownSelect extends React.Component<Props, State> {
  ...
  tagsRef: {current: null | HTMLDivElement;} | null | undefined;
  ...
  render(): React.ReactElement<any> {
    ...
    return <span className={cn("root")} style={{ width }} ref={this.containerRef}>
                <RenderLayer onClickOutside={this.handleClickOutside} onFocusOutside={this.handleFocusOutside} active={opened}>
                    {
          /* eslint-disable-next-line */
        }
                    <div className={cn("wrapper")}>
                        {this.renderInput()}
                        {opened && <DropdownContainer align="left" getParent={() => this.containerRef.current} offsetY={1}>
                                <ScrollContainer maxHeight={300}>
                                    <div className={cn("tags-menu")} style={{ width }} ref={this.tagsRef}>
                                        {filtredTags.length > 0 || allowCreateNewTags ? <div className={cn("tag-list")}>
                                                {filtredTags.map((tag, i) => <Tag key={tag} focus={i === focusedIndex - 1} title={tag} onClick={() => this.selectTag(tag)} />)}
                                                {allowCreateNewTags && !this.tagExists(inputValue) && inputValue.trim() !== "" && <NewTagBadge title={inputValue.trim()} focus={focusedIndex === filtredTags.length + 1} onClick={() => this.selectTag(inputValue.trim())} />}
                                            </div> : <div className={cn("no-tags")}>
  ...
  renderInput(): React.ReactNode {
    ...
    return <div className={isDisabled ? cn("input-area-disabled") : cn("input-area", { focused: isFocused, error })}>
                {value.length !== 0 && value.map(tag => <span className={cn("tag-wrap")} key={tag}>
                            <Tag title={tag} onRemove={() => this.handleRemoveTag(tag)} />
                        </span>)}
                <input className={cn("input")} value={inputValue} onKeyDown={(event: KeyboardEvent) => event.target instanceof HTMLInputElement ? this.handleKeyDown(event.key, event.target.selectionStart) : null} onChange={(event: Event) => event.target instanceof HTMLInputElement ? this.setState({ inputValue: event.target.value, focusedIndex: 0 }) : null} onFocus={() => this.setState({ isFocused: true })} disabled={isDisabled} />
            </div>;
  }
}