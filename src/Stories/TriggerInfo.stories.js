...
var stories = [{
        ...
        triggerState: __assign({}, triggerState),
        data: __assign(__assign({}, sourceData), { desc: "Some list:\n- Line 1\n- Line 2\n- Line 3" })
    }, {
        ...
        triggerState: __assign(__assign({}, triggerState), { maintenance: Date.now() / 1000 + 3600 }),
        data: __assign({}, sourceData)
    }, {
        title: "With maintenance and maintenance info",
        triggerState: __assign(__assign({}, triggerState), { maintenance: Date.now() / 1000 + 3600, maintenanceInfo: {
                setup_user: "Batman",
                setup_time: 1553158221
            } }),
        data: __assign({}, sourceData)
    }];
var story = react_1.storiesOf("TriggerInfo", module).addDecorator(storybook_react_router_1.default());
stories.forEach(function (_a) {
    var title = _a.title, data = _a.data, state = _a.triggerState;
    story.add(title, function () { return React.createElement(TriggerInfo_1.default, { supportEmail: "support@mail.ru", triggerState: state, data: data, onThrottlingRemove: addon_actions_1.action("onThrottlingRemove"), onSetMaintenance: addon_actions_1.action("onSetMaintenance") }); });
});
