...
export default class ScheduleEdit extends React.Component<Props, State> {
  ...
  render(): React.ReactNode {
    ...
    return <div>
          ...
          name,
          enabled
        }, i) => <Checkbox key={name} checked={enabled} onValueChange={checked => onChange({
          ...schedule,
          days: [...days.slice(0, i), { name, enabled: checked }, ...days.slice(i + 1)]
                ...
                <div className={cn("group")}>
                    <span className={cn("radio")}>
                        <Radio checked={allDay} onValueChange={() => {
            onChange({
              ...schedule,
              startOffset: 0,
              endOffset: 1439
            ...
            this.setState({ allDay: true });
                    ...
                    <span className={cn("radio")}>
                        <Radio checked={!allDay} value="specific_interval" onValueChange={() => this.setState({ allDay: false })}>
                            At specific interval
                        </Radio>
                        <Input value={ScheduleEdit.formatTime(startOffset)} width={60} mask="99:99" disabled={allDay} onValueChange={value => onChange({
            ...schedule,
            startOffset: ScheduleEdit.parseTime(value)
          })} />
                        <span>—</span>
                        <Input value={ScheduleEdit.formatTime(endOffset)} width={60} mask="99:99" disabled={allDay} onValueChange={value => onChange({ ...schedule, endOffset: ScheduleEdit.parseTime(value) })} />
                        <Tooltip pos="top right" render={ScheduleEdit.renderTimeRangeHelp} trigger="click">
