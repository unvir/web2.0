...
export default class ContactList extends React.Component<Props, State> {
  ...
  static renderContactIcon(type: string): React.ReactElement<any> {
                ...
                {items.length > 0 ? <div>
                        ...
                        <div className={cn("items-cotnainer")}>
                            <table className={cn("items")}>
                                <tbody>
                                    {items.map(contact => <tr key={contact.id} className={cn("item")} onClick={() => this.handleBeginEditContact(contact)}>
