...
export default function NotificationList(props: Props): React.ReactElement<any> {
  const {
    items,
    onRemove
  } = props;

  return Object.keys(items).length === 0 ? <div className={cn("no-result")}>Empty :-)</div> : <Gapped gap={30} vertical>
      ...
      return <div key={key} className={cn("row")}>
                        ...
                        <div className={cn("state")}>
                            <div className={cn("prev-state")}>
                                <StatusIndicator statuses={uniq(items[key].map(n => n.event.old_state))} size={14} />
                            </div>
                            <div className={cn("arrow")}>
                                <ArrowBoldRightIcon />
                            </div>
                            <div className={cn("curr-state")}>
                                <StatusIndicator statuses={uniq(items[key].map(n => n.event.state))} size={14} />
                            </div>
                        </div>
                        <div className={cn("trigger")}>
                            {id ? <RouterLink to={getPageLink("trigger", id)}>{name}</RouterLink> : <span>&mdash;</span>}
                        </div>
                        <div className={cn("user")}>{user}</div>
