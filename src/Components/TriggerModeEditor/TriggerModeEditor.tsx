...
class TriggerModeEditor extends React.Component<Props, State> {
  ...
  render() {
    ...
    return <React.Fragment>
                ...
                {mode === "simple" && <TriggerSimpleModeEditor watchFor={watchFor} risingValues={risingValues} fallingValues={fallingValues} onChange={this.handleInputChange} onSwitch={this.handleRadioChange} />}
                {mode === "advanced" && <RowStack gap={2} verticalAlign="baseline" block>
                        <Fill>
                            <ValidationWrapperV1 validationInfo={validateExpression(expression, "Expression can't be empty")} renderMessage={tooltip("right middle")}>
                                <Input width="100%" value={expression} onValueChange={value => onChange({ expression: value })} placeholder="t1 >= 10 ? ERROR : (t1 >= 1 ? WARN : OK)" />
                            </ValidationWrapperV1>
                        </Fill>
                        <Fit>
                            <Tooltip pos="top right" render={this.tooltipExpressionHelp} trigger="click">
  ...
  tooltipExpressionHelp = (): React.ReactNode => <div className={cn("expression-help")}>
