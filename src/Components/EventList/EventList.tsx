...
export default function EventList(props: Props): React.ReactNode {
  ...
  return <section>
            ...
            {Object.keys(items).map(key => <div key={key} className={cn("group")}>
                    {items[key].map(({
        old_state: oldState,
        state,
        timestamp,
        ...
        return <div key={`${key}-${timestamp}`} className={cn("row")}>
                                    <div className={cn("name")}>{i === 0 && key}</div>
                                    <div className={cn("state-change")}>
                                        <div className={cn("prev-value")}>
                                            <MetricValues value={oldValue} values={oldValues} placeholder={false} />
                                        </div>
                                        ...
                                        <div className={cn("curr-value")}>
                                            <MetricValues value={value} values={values} placeholder={false} />
                                        </div>
                                    </div>
