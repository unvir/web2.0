...
export default class ContactSelect extends React.Component<Props> {
  ...
  render(): React.ReactNode {
    ...
    return <div onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
                {contactIds.map(x => availableContacts.find(contact => contact.id === x)).filter(Boolean).map(x => <div key={x.id} className={cn("contact")}>
                ...
                <div>
                    <ComboBox error={error} warning={warning} width="100%" onFocus={onFocus} onBlur={onBlur} onValueChange={this.handleChangeContactToAdd} getItems={this.getContactsForComboBox} placeholder="Select delivery channel" renderNotFound={() => "No delivery channels found"} renderItem={item => <React.Fragment>
  ...
  handleChangeContactToAdd = <T extends {value: string;label: string;}>(value: T) => {
    const {
      onChange,
      contactIds
    } = this.props;
  ...
  getContactsForComboBox = async (query: string): Promise<Array<{value: string;label: string;type: string;}>> => {
    const {
      contactIds,
      availableContacts
    } = this.props;
    return availableContacts.filter(x => !contactIds.includes(x.id)).filter(x => ContactSelect.isContactMatch(x, query)).slice(0, 10).map(x => ({
      value: x.id,
      label: x.value,
      type: x.type
    }));
