...
export default class NewContactModal extends React.Component<Props, State> {
  ...
  render(): React.ReactNode {
    ...
    const idActionButtonsDisabled = !value || !type || createInProcess || createAndTestInProcess;
    ...
    return <Modal onClose={onCancel} ignoreBackgroundClick>
                ...
                <Modal.Body>
                    <ValidationContainer ref={this.validationContainer}>
                        <ContactEditForm contactDescriptions={contactDescriptions} contactInfo={contactInfo} onChange={this.handleChange} />
                    </ValidationContainer>
                </Modal.Body>
                <Modal.Footer panel sticky>
                    <ModalError message={error} maxWidth="450px" />
                    <RowStack gap={2} block baseline>
                        <Button use="primary" loading={createInProcess} disabled={idActionButtonsDisabled} onClick={() => {
            this.handleCreateContact();
          }}>
                            Add
                        </Button>
                        <Button disabled={idActionButtonsDisabled} loading={createAndTestInProcess} onClick={() => {
  ...
  handleImport = (fileData: string, fileName: string) => {
    ...
    try {
      ...
      if (contactDescriptions.every(({
        type
      }) => type !== newContact.type)) {
        throw new Error(`Type must be one of ${contactDescriptions.map(({
          type
        }) => type).join(", ")}`);
      }
