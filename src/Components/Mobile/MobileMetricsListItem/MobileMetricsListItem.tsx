...
import { Maintenances, Maintenance, getMaintenanceCaption } from "../../../Domain/Maintenance";
...
export default class MobileMetricsListItem extends React.Component<Props, State> {
  ...
  render(): React.ReactNode {
    ...
    return <div className={cn("root", { deleted })}>
                <div className={cn("status")}>
                    {this.renderStatus()}
                    {isUnderkMaintenance(value.maintenance) && <div className={cn("maintenance-icon")}>
                            <UserSettingsIcon />
                        </div>}
                </div>
                ...
                <div className={cn("buttons")}>
                    <div className={cn("button-block", { visible: buttonsState === "Default" })}>
                        <button type="button" className={cn("button")} onClick={() => this.setState({ buttonsState: "SelectAction" })}>
                            <MenuDotsIcon />
                        </button>
                    </div>
                    <div className={cn("button-block", { visible: buttonsState === "SelectAction" })}>
                        <button type="button" className={cn("button")} onClick={() => this.setState({ buttonsState: "SetMaintenance" })}>
                            <UserSettingsIcon />
                        </button>
                        <button type="button" onClick={this.handleDeleteMetric} className={cn("button")}>
                            <TrashIcon />
                        </button>
                        <button type="button" className={cn("button")} onClick={() => this.setState({ buttonsState: "Default" })}>
                            <RedoIcon />
                        </button>
                    </div>
                    {buttonsState === "SetMaintenance" && <Modal noClose onClose={() => this.setState({ buttonsState: "Default" })}>
                            <Modal.Body>
