...
export default function TriggerList(props: Props): React.ReactNode {
  ...
  return <div>
            {items.length === 0 ? <div className={cn("no-result")}>No results :-(</div> : items.map(item => <div className={cn("item")} key={item.id}>
                        <TriggerListItem searchMode={searchMode} data={item} onChange={onChange && ((metric, maintenance) => onChange(item.id, metric, maintenance))} onRemove={onRemove && (metric => onRemove(item.id, metric))} />
                    </div>)}
        </div>;
}