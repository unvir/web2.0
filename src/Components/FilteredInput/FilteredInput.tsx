...
function usedForDestructuring(_value: any) {}
...
export default class FilteredInput<T> extends React.Component<FilteredInputProps<T>, FilteredInputState> {
  ...
  refs: {
    innerInput: HTMLInputElement | null | undefined;
    testWidth: HTMLElement | null | undefined;
  ...
  getHintState(hintValue: string | null | undefined, displayValue: string | null | undefined): $Shape<HintState> | null | undefined {
  ...
  render(): React.ReactNode {
    ...
    return <span className={cn("root")} ref="root">
                <div className={cn("test-width")} ref="testWidth" />
                {this.state.hintValue && hintClip !== null && hintClip !== undefined && <div className={cn("hint-container")} style={{ clip: `rect(auto,${hintClip}px,auto,0px)` }}>
                        <Input {...restProps} disabled borderless placeholder={this.state.hintValue ? this.state.hintValue : ""} value="" />
                    </div>}
                <Input {...restProps} ref="innerInput" value={this.state.displayValue || ""} onValueChange={value => this.handleInputChange(value)} // I suggested to rename properties from on*Change to on*ChangeHandler which not about DOM Events to make understandable that its not React event handler. (I know its component from kontur library but we can change same in moira components)
      onBlur={(event, ...rest) => {
        this.focused = false;
