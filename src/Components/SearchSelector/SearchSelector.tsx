...
class SearchSelector extends React.Component<Props, State> {
  ...
  render() {
    ...
    return <Selector search={searchText} tokens={selectedTokens} renderToken={this.renderToken} onEnterKeyDown={this.handleEnterKeyDown} onBackspaceKeyDown={this.onRemoveLastToken} onInputChange={this.handleInputChange}>
                <div className={cn("container")}>
                    {clearedSearchValue === "" ? <SelectorInitialView tokens={subscribedTokens} onSelect={this.handleTokenSelect} /> : <SelectorResultsView tokens={resultTokens} onSelect={this.handleTokenSelect} />}
                </div>
            </Selector>;
  }
  ...
  onRemoveLastToken = () => {
    ...
    onChange([...selectedTokens.slice(0, index), ...selectedTokens.slice(index + 1)], clearedSearchValue);
  };
  ...
  renderToken = (token: string) => <Token type="removable" onRemove={this.handleTokenRemove}>
            {token}
        </Token>;
}
