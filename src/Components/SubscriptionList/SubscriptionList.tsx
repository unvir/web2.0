...
import { SubscriptionInfo } from "../SubscriptionEditor/SubscriptionEditor";
import cn from "./SubscriptionList.less";

export { SubscriptionInfo };

type Props = {
  onAddSubscription: (arg0: SubscriptionInfo) => Promise<Subscription | null | undefined>;
  onRemoveSubscription: (arg0: Subscription) => Promise<void>;
  onUpdateSubscription: (arg0: Subscription) => Promise<void>;
  onTestSubscription: (arg0: Subscription) => Promise<void>;
  tags: Array<string>;
...
type State = {
  ...
  newSubscription: SubscriptionInfo | null | undefined;
  subscriptionEditModalVisible: boolean;
  subscriptionToEdit: Subscription | null | undefined;
};
...
export default class SubscriptionList extends React.Component<Props, State> {
  ...
  render(): React.ReactElement<any> {
    ...
    return <div>
                {subscriptions.length > 0 ? <div>
                    ...
                    </div> : this.renderAddSubscriptionMessage()}
                {newSubscriptionModalVisible && newSubscription != null && <CreateSubscriptionModal subscription={newSubscription} tags={tags} contacts={contacts} onChange={update => this.setState({ newSubscription: { ...newSubscription, ...update } })} onCancel={() => this.setState({ newSubscriptionModalVisible: false })} onCreateSubscription={this.handleCreateSubscription} onCreateAndTestSubscription={this.handleCreateAndTestSubscription} />}
                {subscriptionEditModalVisible && subscriptionToEdit != null && <SubscriptionEditModal subscription={subscriptionToEdit} tags={tags} contacts={contacts} onChange={update => this.setState({
        subscriptionToEdit: { ...subscriptionToEdit, ...update }
      })} onCancel={() => this.setState({ subscriptionEditModalVisible: false })} onUpdateSubscription={this.handleUpdateSubscription} onUpdateAndTestSubscription={this.handleUpdateAndTestSubscription} onRemoveSubscription={this.handleRemoveSubscription} />}
            </div>;
  }
  ...
  renderSubscriptionRow(subscription: Subscription): React.ReactNode {
    const {
      contacts
    } = this.props;
    return <tr key={subscription.id} className={cn("item")} onClick={() => this.handleEditSubscription(subscription)}>
                <td className={cn("tags-cell")}>
                    <TagGroup tags={subscription.tags} />
                </td>
                <td className={cn("contacts-cell")}>
                    <Gapped gap={10}>
                        {subscription.contacts.map(x => contacts.find(y => y.id === x)).filter(Boolean).map(x => <ContactInfo key={x.id} contact={x} />)}
                    </Gapped>
                </td>
  ...
  renderAddSubscriptionMessage(): React.ReactNode {
    return <Center>
