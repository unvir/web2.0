...
type Props = {
  data: $Shape<Trigger>;
  tags: Array<string>;
  onChange: (arg0: $Shape<Trigger>) => void;
...
export default class TriggerEditForm extends React.Component<Props, State> {
  ...
  static validateRequiredString(value: string, message?: string): ValidationInfo | null | undefined {
  ...
  static validateRequiredNumber(value: number | null | undefined): ValidationInfo | null | undefined {
  ...
  static validateTTL(value: number | null | undefined): ValidationInfo | null | undefined {
  ...
  render(): React.ReactNode {
    ...
    return <Form>
                <FormRow label="Name">
                    <ValidationWrapperV1 validationInfo={TriggerEditForm.validateRequiredString(name)} renderMessage={tooltip("right middle")}>
                        <Input width="100%" value={name} onValueChange={value => onChange({ name: value })} />
                    </ValidationWrapperV1>
                </FormRow>
                <FormRow label="Description" useTopAlignForLabel>
                    <div className={cn("description-mode-tabs")}>
                        <Tabs value={descriptionMode} onValueChange={value => this.setState({ descriptionMode: value })}>
                            <Tabs.Tab id="edit">Edit</Tabs.Tab>
                            <Tabs.Tab id="preview">Preview</Tabs.Tab>
                        </Tabs>
                    </div>
                    {descriptionMode === "edit" ? <>
                            <Textarea width="100%" value={desc || ""} onValueChange={value => onChange({ desc: value })} />
                            <EditDescriptionHelp />
                        </> : <div className={cn("wysiwyg", "description-preview")} dangerouslySetInnerHTML={{
        ...
        <div key={`target-${i}`} className={cn("target")}>
                            ...
                            <RowStack block verticalAlign="baseline" gap={1}>
                                <Fill>
                                    <ValidationWrapperV1 validationInfo={TriggerEditForm.validateRequiredString(x)} renderMessage={tooltip("right middle")}>
                                        <Input width="100%" value={x} onValueChange={value => this.handleUpdateTarget(i, value)} />
                                    </ValidationWrapperV1>
                                </Fill>
                                {targets.length > 1 && <Fit>
                                        <Checkbox checked={aloneMetrics !== undefined && aloneMetrics !== null && aloneMetrics[`t${i + 1}`]} onValueChange={value => this.handleUpdateAloneMetrics(i, value)}>
                                            Single
                                        </Checkbox>
                                    </Fit>}
                                {targets.length > 1 && <Fit>
                ...
                <FormRow>
                    <TriggerModeEditor triggerType={triggerType} value={{ error_value: data.error_value, warn_value: data.warn_value }} expression={expression} validateExpression={TriggerEditForm.validateRequiredString} disableSimpleMode={targets.length > 1} onChange={value => onChange(value)} />
                </FormRow>

                <FormRow singleLineControlGroup>
                    <StatusSelect value={ttlState} availableStatuses={Object.keys(Statuses).filter(x => x !== Statuses.EXCEPTION)} onChange={value => onChange({ ttl_state: value })} />
                    <span>if has no value for</span>
                    <ValidationWrapperV1 validationInfo={TriggerEditForm.validateTTL(ttl)} renderMessage={tooltip("right middle")}>
                        <FormattedNumberInput width={80} value={typeof ttl === "number" ? ttl : null} editFormat={defaultNumberEditFormat} viewFormat={defaultNumberViewFormat} onChange={value => onChange({ ttl: value || 0 })} />
                    </ValidationWrapperV1>
                    <span>seconds</span>
                </FormRow>
                <FormRow singleLineControlGroup>
                    <Checkbox checked={muteNewMetrics} onValueChange={checked => onChange({ mute_new_metrics: checked })}>
                ...
                <FormRow label="Tags" useTopAlignForLabel>
                    <ValidationWrapperV1 validationInfo={tags.length === 0 ? {
          type: "submit",
          message: "Select at least one tag"
        } : null} renderMessage={tooltip("right top")}>
                        <TagDropdownSelect allowCreateNewTags value={tags} availableTags={allTags} width={650} onChange={selectedTags => onChange({
            tags: selectedTags
          })} />
                    </ValidationWrapperV1>
                </FormRow>
                {remoteAllowed && <FormRow label="Data source" singleLineControlGroup>
                        <RadioGroup name="data-source" defaultValue={!isRemote ? TriggerDataSources.LOCAL : TriggerDataSources.GRAPHITE} onValueChange={value => onChange({ is_remote: value !== TriggerDataSources.LOCAL })}>
  ...
  handleRemoveTarget(targetIndex: number) {
    ...
    for (let i = 0; i < targets.length; i += 1) {
      const target = `t${i + 1}`;
      aloneMetricsIndex[i] = aloneMetrics !== undefined && aloneMetrics !== null && aloneMetrics[target];
    }

    const newAloneMetricsIndex = [...aloneMetricsIndex.slice(0, targetIndex), ...aloneMetricsIndex.slice(targetIndex + 1)];
...
function FormRow({
  ...
  return <div className={cn("row")}>
            {label != null && <div className={cn("label", { "label-for-group": useTopAlignForLabel })}>
                    {label}
                </div>}
