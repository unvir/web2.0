...
export default class SubscriptionEditModal extends React.Component<Props, State> {
  ...
  render(): React.ReactNode {
    ...
    const {
      updateInProcess,
      updateAndTestInProcess,
      deleteInProcess
    } = this.state;
    const isActionButtonsDisabled = updateInProcess || updateAndTestInProcess || deleteInProcess;
    return <Modal ignoreBackgroundClick onClose={onCancel}>
                <Modal.Header sticky={false}>Subscription editing</Modal.Header>
                <Modal.Body>
                    <ValidationContainer ref={this.validationContainer}>
                        <SubscriptionEditor subscription={subscription} onChange={onChange} tags={tags} contacts={contacts} />
                    </ValidationContainer>
                </Modal.Body>
                <Modal.Footer panel sticky>
                    <RowStack gap={2} block baseline>
                        <Button use="primary" disabled={isActionButtonsDisabled} loading={updateInProcess} onClick={this.handleUpdate}>
                            Save
                        </Button>
                        <Button disabled={isActionButtonsDisabled} loading={updateAndTestInProcess} onClick={this.handleUpdateAndTest}>
                            Save and test
                        </Button>
                        <FileExport title={this.getFileName()} data={omitSubscription(subscription)}>
                            Export
                        </FileExport>
                        <Fill />
                        <Button use="danger" disabled={isActionButtonsDisabled} loading={deleteInProcess} onClick={this.handleDelete}>
                            Delete
                        </Button>
                    </RowStack>
                </Modal.Footer>
  ...
  getFileName = () => {
    ...
    return `subscription ${contactValues.join(" ")} ${subscription.tags.slice(0, 5).map(t => t.slice(0, 8)).join(" ")}`;
  };
}