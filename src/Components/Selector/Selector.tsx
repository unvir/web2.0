...
const Dropdown = ({
  anchor,
  children
}: {anchor: HTMLLabelElement | null | undefined;children: React.ReactNode;}) => {
  ...
  render() {
    ...
    return <>
                <Foco className={cn("wrapper")} onClickOutside={this.closeDropdown}>
                    <label className={cn({ selector: true, focused })} htmlFor="selector" ref={this.dropdownAnchorRef}>
                        {tokens.map(token => <span key={token} className={cn("token-container")}>
                                {renderToken(token)}
                            </span>)}
                        <input className={cn("input")} id="selector" type="text" value={search} autoComplete="off" ref={this.searchInputRef} onChange={this.handleInputChange} onKeyDown={this.handleInputKeyDown} onFocus={this.openDropdown} />
                    </label>
