...
class TriggerPage extends React.Component<Props, State> {
  ...
  async loadData() {
    ...
    const {
      page
    } = TriggerPage.parseLocationSearch(location.search);
    const {
      id
    } = match.params;
    ...
    try {
      const [trigger, triggerState, triggerEvents] = await Promise.all([moiraApi.getTrigger(id, { populated: true }), moiraApi.getTriggerState(id), moiraApi.getTriggerEvents(id, transformPageFromHumanToProgrammer(page))]);
  ...
  changeLocationSearch = update => {
    ...
    history.push(`?${queryString.stringify({ ...locationSearch, ...update }, {
      arrayFormat: "index",
      encode: true
    })}`);
  };
