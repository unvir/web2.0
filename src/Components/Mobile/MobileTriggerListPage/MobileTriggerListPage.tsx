  ...
  render(): React.ReactNode {
    ...
    return <div>
                ...
                <div className={cn("content")}>
                    {triggers != null && triggers.length === 0 && <div style={{ padding: 30, color: "#666", textAlign: "center" }}>
                            No results :-(
                        </div>}
                    {triggers != null && triggers.map(trigger => <MobileTriggerListItem key={trigger.id} data={trigger} />)}
                    {triggers != null && loading && <div>
                            <Spinner type="mini" caption="Loading..." />
                        </div>}
                </div>
                {pageCount > 1 && <div style={{ padding: "25px 15px 50px" }}>
                        <Paging caption="Next page" activePage={activePage} pagesCount={pageCount} onPageChange={page => {
          if (onChange) {
            onChange({ page });
          }
