...
export default function MetricList(props: Props): React.ReactNode {
  const {
    status,
    items,
    onSort,
  ...
  return <section className={cn("table")}>
            <header className={cn("row", "header")}>
                {status && <div className={cn("state")} />}
                <div className={cn("name")}>
                    <button type="button" className={cn("a11y-span", { sorting: onSort })} onClick={onSort && (() => onSort("name"))}>
                        Name
                        {sortingColumn === "name" && <span className={cn("icon")}>{sortingIcon}</span>}
                    </button>
                </div>
                <div className={cn("event")}>
                    <button type="button" className={cn("a11y-span", { sorting: onSort })} onClick={onSort && (() => onSort("event"))}>
                        Last event{" "}
                        {sortingColumn === "event" && <span className={cn("icon")}>{sortingIcon}</span>}
                    </button>
                </div>
                <div className={cn("value")}>
                    <button type="button" className={cn("a11y-span", { sorting: onSort })} onClick={onSort && (() => onSort("value"))}>
                        Value{" "}
                        {sortingColumn === "value" && <span className={cn("icon")}>{sortingIcon}</span>}
                    </button>
                </div>
                <div className={cn("maintenance")} />
                <div className={cn("author")} />
                <div className={cn("delete")}>
                    {typeof noDataMerticCount === "number" && noDataMerticCount > 1 && onNoDataRemove && <Button use="link" icon={<TrashIcon />} onClick={() => onNoDataRemove()}>
                                Delete all NODATA
                            </Button>}
                </div>
            </header>
        ...
        return <div key={metric} className={cn("row")}>
                            {status && <div className={cn("state")}>
                            ...
                            <div className={cn("value")}>
                                <MetricValues value={value} values={values} placeholder hideTargetsNames={hideTargetsNames} />
                            </div>
                            <div className={cn("maintenance")}>
                                <Dropdown use="link" caption={maintenanceCaption(delta)}>
                                    {Object.keys(Maintenances).map(key => <MenuItem key={key} onClick={() => onChange(metric, key)}>
                                            {getMaintenanceCaption(key)}
                                        </MenuItem>)}
                                </Dropdown>
                            </div>
                            <div className={cn("author")}>
                                {delta > 0 && maintenanceInfo && maintenanceInfo.setup_user && maintenanceInfo.setup_time && <Tooltip render={() => <div>
                                                    Maintenance was set
                                                    <br />
                                                    by {maintenanceInfo.setup_user}
                                                    <br />
                                                    at{" "}
                                                    {format(fromUnixTime(maintenanceInfo.setup_time || 0), "MMMM d, HH:mm:ss")}
                                                </div>}>
