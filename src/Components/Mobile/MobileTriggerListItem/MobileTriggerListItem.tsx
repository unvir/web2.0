...
export default class TriggerListItem extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    super(props);
    this.state = {
      groupedMetrics: TriggerListItem.groupMetricsByStatuses((props.data.last_check || {}).metrics)
    };
  }

  static groupMetricsByStatuses(metrics: MetricList): Record<Status, Record<string, Metric>> {
  ...
  render(): React.ReactNode {
    ...
    return <ReactRouterLink className={cn("root")} to={getPageLink("trigger", id)}>
                <div className={cn("status")}>
                    {this.renderStatus()}
                    {throttling !== 0 && <div className={cn("throttling-flag")}>
                            <FlagSolidIcon />
                        </div>}
                    {delta > 0 && <div className={cn("maintenance")}>
                            <UserSettingsIcon />
                        </div>}
                </div>
  ...
  renderCounters(): React.ReactNode {
    const counters = Object.keys(Statuses).map(status => ({
      status,
    ...
    return <span className={cn("metrics-list")}>
  ...
  renderStatus(): React.ReactNode {
    ...
    const metricStatuses = Object.keys(Statuses).filter(x => Object.keys(this.filterMetricsByStatus(x)).length !== 0);
  ...
  filterMetricsByStatus(status: Status): Record<string, Metric> {
    const {
      groupedMetrics
    } = this.state;
