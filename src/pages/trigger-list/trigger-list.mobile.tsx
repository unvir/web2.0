...
class TriggerListMobile extends React.Component<Props, State> {
  ...
  render() {
    ...
    if (showTagSelector) {
      return <MobileTagSelectorPage availableTags={allTags} selectedTags={selectedTags} onlyProblems={onlyProblems} onClose={() => this.setState({ showTagSelector: false })} onChange={this.handleChangeSelectedTags} />;
    }

    return <MobileTriggerListPage selectedTags={selectedTags} triggers={triggers} loading={false} onOpenTagSelector={() => this.setState({ showTagSelector: true })} activePage={activePage} pageCount={pageCount} onChange={onChange} />;
  }
