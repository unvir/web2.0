  ...
  render(): React.ReactNode {
    ...
    return <div className={cn("form")}>
                <div className={cn("row")}>
                    <Select placeholder="Select channel type" width="100%" value={type || null} renderItem={(v, item) => <span>
                                {v && <ContactTypeIcon type={v} />} {item}
                            </span>} renderValue={(v, item) => <span>
                                {v && <ContactTypeIcon type={v} />} {item}
                            </span>} onValueChange={v => {
          ...
          v && onChange({ type: v }); // eslint-disable-line
                ...
                <div className={cn("row")}>
                    <ValidationWrapperV1 renderMessage={tooltip("top left")} validationInfo={this.validateValue()}>
                        <Input width="100%" disabled={type == null} placeholder={(currentContactConfig && currentContactConfig.placeholder) || ""} value={value} onValueChange={v => onChange({ value: v })} />
                    </ValidationWrapperV1>
                </div>
