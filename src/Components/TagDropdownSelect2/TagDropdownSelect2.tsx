...
export default class TagDropdownSelect extends React.Component<Props, State> {
                                ...
                                <div className={cn("menu-container")}>
                                    <ScrollContainer maxHeight={300}>
                                        <div className={cn("tags-menu")} ref={this.tagsRef}>
                                            {subscribed.length !== 0 && value.length === 0 && <div className={cn("group")}>
                                                    <b className={cn("title")}>Subscriptions</b>
                                                    <TagGroup tags={subscribed} onClick={tag => this.selectTag(tag)} />
                                                </div>}
                                            {remained.length !== 0 && value.length === 0 && <div className={cn("group")}>
                                                    <b className={cn("title")}>All tags</b>
                                                    <TagGroup tags={remained} onClick={tag => this.selectTag(tag)} />
                                                </div>}
                                            {value.length !== 0 && <div className={cn("group")}>
                                                    <b className={cn("title")}>Search results</b>
                                                    <div className={cn("tag-list")}>
                                                        {filtredTags.length > 0 ? filtredTags.map((tag, i) => <Tag key={tag} focus={i === focusedIndex - 1} title={tag} onClick={() => this.selectTag(tag)} />) : <div className={cn("no-tags")}>
  ...
  renderInput(): React.ReactNode {
    ...
    return <div className={cn("input-area", { focused: isFocused })}>
                {selected.length !== 0 && selected.map(tag => <span className={cn("tag-wrap")} key={tag}>
                            <Tag title={tag} onRemove={() => this.handleRemoveTag(tag)} />
                        </span>)}
                <input className={cn("input")} value={value} onKeyDown={(event: KeyboardEvent) => event.target instanceof HTMLInputElement ? this.handleKeyDown(event.key, event.target.selectionStart) : null} onChange={(event: Event) => event.target instanceof HTMLInputElement ? this.setState({
        value: event.target.value.replace(/\s/gi, ""),
        focusedIndex: 0
      }) : null} onFocus={() => this.setState({ isFocused: true })} />
            </div>;
  }
