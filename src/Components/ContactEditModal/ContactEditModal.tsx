...
type Props = {
  ...
  contactInfo: Contact;
  onChange: (arg0: $Shape<Contact>) => void;
  onCancel: () => void;
...
export default class ContactEditModal extends React.Component<Props, State> {
  ...
  render(): React.ReactNode {
    ...
    return <Modal onClose={onCancel} ignoreBackgroundClick>
                <Modal.Header sticky={false}>Delivery channel editing</Modal.Header>
                <Modal.Body>
                    <ValidationContainer ref={this.validationContainer}>
                        <ContactEditForm contactDescriptions={contactDescriptions} contactInfo={contactInfo} onChange={update => onChange({ ...contactInfo, ...update })} />
                    </ValidationContainer>
                </Modal.Body>
                <Modal.Footer panel sticky>
                    <RowStack gap={2} block baseline>
                        <Button use="primary" disabled={isActionButtonDisabled} loading={updateInProcess} onClick={this.handleUpdateContact}>
                            Save
                        </Button>
                        <Button loading={updateAndTestInProcess} disabled={isActionButtonDisabled} onClick={this.handleUpdateAndTestContact}>
                            Save and test
                        </Button>
                        <FileExport title={`delivery channel ${contactInfo.type} ${contactInfo.value}`} data={omitContact(contactInfo)}>
                            Export
                        </FileExport>
                        <Fill />
                        <Button use="danger" loading={deleteInProcess} disabled={isActionButtonDisabled} onClick={this.handleDeleteContact}>
                            Delete
                        </Button>
                    </RowStack>
                </Modal.Footer>
