...
import SubscriptionList, { SubscriptionInfo } from "../Components/SubscriptionList/SubscriptionList";
...
class SettingsContainer extends React.Component<Props, State> {
  ...
  render(): React.ReactNode {
    ...
    return <Layout loading={loading} error={error}>
                <LayoutContent>
                    ...
                    {config != null && settings != null && settings.contacts != null && <div style={{ marginBottom: 50 }}>
                            <ContactList contactDescriptions={config.contacts} items={settings.contacts} onTestContact={this.handleTestContact} onAddContact={this.handleAddContact} onUpdateContact={this.handleUpdateContact} onRemoveContact={this.handleRemoveContact} />
                        </div>}
                    {settings != null && tags != null && settings.subscriptions != null && settings.contacts != null && settings.contacts.length > 0 && <SubscriptionList tags={tags} contacts={settings.contacts} subscriptions={settings.subscriptions} onTestSubscription={this.handleTestSubscription} onAddSubscription={this.handleAddSubscription} onRemoveSubscription={this.handleRemoveSubscription} onUpdateSubscription={this.handleUpdateSubscription} />}
                </LayoutContent>
            </Layout>;
  }
  ...
  handleAddContact = async (contact: $Shape<Contact>): Promise<Contact | null | undefined> => {
    const {
      moiraApi
    } = this.props;
    const {
    ...
    try {
      ...
      newContact = {
        ...newContact,
        value: SettingsContainer.normalizeContactValueForUi(newContact.type, newContact.value)
      };
      this.setState({
        settings: {
          ...settings,
  ...
  handleAddSubscription = async (subscription: SubscriptionInfo): Promise<Subscription | null | undefined> => {
    const {
      moiraApi
    } = this.props;
    const {
