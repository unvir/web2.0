...
const story = storiesOf("TriggerListItem", module).addDecorator(StoryRouter()).addParameters({
  creevey: {
    tests: {
      async States() {
        await this.browser.actions({ bridge: true }).move({
        ...
        await this.browser.actions({ bridge: true }).click(this.browser.findElement({
          css: 'div[data-tid="TriggerListItem_status"]'
        })).perform();
...
stories.forEach(({
  ...
  story.add(title, () => <TriggerListItem searchMode={false} data={data} onChange={action("onChange")} onRemove={action("onRemove")} />);
});