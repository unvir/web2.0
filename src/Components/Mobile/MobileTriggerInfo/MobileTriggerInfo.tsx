...
import { Maintenances, getMaintenanceCaption, Maintenance } from "../../../Domain/Maintenance";
...
function ScheduleView(props: {data: Schedule;}): React.ReactNode {
  const {
    data
  } = props;
  const {
  ...
  return <span>
            {days.length === enabledDays.length ? "Everyday" : enabledDays.map(({
      name
    }) => name).join(", ")}{" "}
...
export default class MobileTriggerInfo extends React.Component<Props, State> {
  ...
  render(): React.ReactNode {
    ...
    return <div>
                <MobileHeader color={this.getHeaderColor()}>
                    <Sticky side="top">
                        <MobileHeader.HeaderBlock color={this.getHeaderColor()}>
                            ...
                            <MobileHeader.RightButton icon={<UserSettingsIcon />} onClick={() => this.setState({ showMaintenance: true })} />
                        </MobileHeader.HeaderBlock>
                    </Sticky>
                    <MobileHeader.DetailsBlock>
                        {trigger != null && <div className={cn("info")}>
                                <div className={cn("plain-row", "description")}>
                                    {trigger.desc && trigger.desc}
                                </div>
                                {sched != null && <div className={cn("form-row")}>
                                ...
                                {maintenance && <div className={cn("form-row")}>
                                        <div className={cn("caption")}>Maintenance:</div>
                                        <div className={cn("value")}>
                                            {checkMaintenance(maintenance)}
                                        </div>
                                    </div>}
                                {(trigger.throttling !== 0 || showThrottling) && <div className={cn("plain-row")}>
                                        <FlagSolidIcon />{" "}
                                        {trigger.throttling !== 0 ? "Throttling enabled." : "No Throttling."}{" "}
                                        <BorderlessButton onClick={this.handleDisableThrottling} disabled={trigger.throttling === 0}>
                                            Disable
                                        </BorderlessButton>
                                    </div>}
                                {msg != null && msg !== "" && <div className={cn("form-row")}>
                ...
                {showMaintenance && <Modal noClose onClose={() => this.setState({ showMaintenance: false })}>
                        <Modal.Body>
                            <div className={cn("modal-content")}>
                                <div className={cn("modal-header")}>Maintenance trigger</div>
                                {Object.keys(Maintenances).map(key => <button type="button" onClick={() => this.handleSetMaintenance(Maintenances[key])} className={cn("modal-button")} key={key}>
  ...
  getWorstTriggerState(): Status | null | undefined {
    ...
    const metricStatuses = Object.keys(Statuses).filter(x => Object.keys(metrics).map(y => metrics[y].state).includes(x));
...
function BorderlessButton({
  ...
  disabled,
  onClick
}: BorderlessButtonProps): React.ReactNode {
  return <button type="button" onClick={onClick} className={cn("borderless-button", { disabled })}>
            <span>{children}</span>
        </button>;
}