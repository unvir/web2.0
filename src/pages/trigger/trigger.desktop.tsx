...
import { SortingColum } from "../../Components/MetricList/MetricList";
...
class TriggerDesktop extends React.Component<Props, State> {
  ...
  static composeEvents(events: Array<Event>, triggerName: string): {
  ...
  render() {
    ...
    const noDataMerticCount = (metrics && Object.keys(metrics).filter(key => metrics[key].state === Statuses.NODATA).length) || 0;

    return <Layout loading={loading} error={error}>
                {trigger && state && <LayoutPlate>
                        <TriggerInfo data={trigger} triggerState={state} onThrottlingRemove={() => disableThrottling(trigger.id)} onSetMaintenance={maintenance => setTriggerMaintenance(trigger.id, maintenance)} />
                    </LayoutPlate>}
                <LayoutContent>
                    {!(isMetrics || isEvents) ? <Center>
                            <span style={{ color: "#888888" }}>
                                There is no metrics evaluated for this trigger.
                            </span>
                        </Center> : // eslint-disable-next-line no-nested-ternary
        <Tabs value={isMetrics ? page > 1 ? "events" : "state" : "state"}>
                            {isMetrics && triggerId && <Tab id="state" label="Current state">
                                    <MetricList status items={this.sortMetrics(metrics)} onSort={sorting => {
            ...
            }} sortingColumn={sortingColumn} sortingDown={sortingDown} onChange={(metric, maintenance) => {
              setMetricMaintenance(triggerId, maintenance, metric);
            }} onRemove={metric => removeMetric(triggerId, metric)} noDataMerticCount={noDataMerticCount} onNoDataRemove={() => removeNoDataMetric(triggerId)} />
                                </Tab>}
                            {isEvents && trigger && trigger.name && <Tab id="events" label="Events history">
                                    <EventList items={TriggerDesktop.composeEvents(events, trigger.name)} />
                                    {pageCount > 1 && <div style={{ paddingTop: 20 }}>
                                            <Paging caption="Next page" activePage={page} pagesCount={pageCount} onPageChange={newPage => onPageChange({ page: newPage })} withoutNavigationHint />
                                        </div>}
                                </Tab>}
                        </Tabs>}
                </LayoutContent>
  ...
  sortMetrics(metrics: {
    ...
    const sorting = {
      ...
      value: (x, y) => {
        const valueA = Number.isFinite(metrics[x].value) && metrics[x].value != null ? metrics[x].value : Number.MIN_SAFE_INTEGER;
        const valueB = Number.isFinite(metrics[y].value) && metrics[y].value != null ? metrics[y].value : Number.MIN_SAFE_INTEGER;
        if (valueA < valueB) {
          return sortingDown ? -1 : 1;
        }
        if (valueA > valueB) {
    ...
    return Object.keys(metrics).sort(sorting[sortingColumn]).reduce((data, key) => ({ ...data, [key]: metrics[key] }), {});
  }
}
